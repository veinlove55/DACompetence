...
def Testing():
    ...
    print('Get agent')
    if p.model_path == "":
        lane_agent = agent.Aefficientnet_pytorchgent()
    else:
        lane_agent = agent.Agent()
        lane_agent.load_weights()
    ...
    if p.mode == 0 : # check model with test data 
        ...
        cap = cv2.VideoCapture("/home/two1szh/PycharmProjects/SWCompetence/1118_1.mov")
        fourcc = cv2.VideoWriter_fourcc(*'MJPG')
        vout = cv2.VideoWriter('demo.avi', fourcc, 30.0, (960, 540))
        while(cap.isOpened()):
            ret, frame = cap.read()
            if frame is None:
                continue
