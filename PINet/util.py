...
from PINet.parameters import Parameters
import math
...
def visualize_points_origin_size(x, y, test_image, ratio_w, ratio_h):
    ...
    for i, j in zip(x, y):
        ...
        for index in range(len(i)):
            cv2.circle(image, (int(i[index]), int(j[index])), 5, p.color[color], -1)
    cv2.imshow("test2", image)
    cv2.waitKey(0)  
    ...
    for y in range(p.grid_y):
        for x in range(p.grid_x):
            if gt_point[0][y][x] > 0:
                ...
                image = cv2.circle(image, (xx, yy), 5, p.color[1], -1)
...
def draw_points(x, y, image):
    ...
    for i, j in zip(x, y):
        ...
        # x position of last point of i
        x_last_point = int(i[0])
        color_line = (0, 255, 0)
        if x_last_point >= x_left_threshold and x_last_point <= x_right_threshold:
            color_line = (0, 0, 255)
        for index in range(len(i)):
            mage = cv2.circle(image, (int(i[index]), int(j[index])), 1, color_line, -1)
